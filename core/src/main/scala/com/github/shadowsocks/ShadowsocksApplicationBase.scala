...
import com.github.shadowsocks.lib.R
...
object ShadowsocksApplicationBase {
  private final val SIG_FUNC = "getSignature"
  var app: ShadowsocksApplicationBase = _
}

class ShadowsocksApplicationBase extends Application {
  import ShadowsocksApplicationBase._
  ...
  lazy val profileManager = new ProfileManager(settings)
      ...
      override def onResult(holder: ContainerHolder) {
        ...
        container.registerFunctionCallMacroCallback(SIG_FUNC,
          (functionName: String, parameters: util.Map[String, AnyRef]) => {
            if (functionName == SIG_FUNC) Utils.getSignature(ShadowsocksApplicationBase.this)
            null
          })
      }
    }
