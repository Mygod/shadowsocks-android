...
import com.github.shadowsocks.ShadowsocksApplicationBase.app
...
class ShadowsocksNotification(private val service: BaseService, profileName: String, visible: Boolean = false) {
  ...
  private val builder = new NotificationCompat.Builder(service)
    ...
    .setContentIntent(PendingIntent.getActivity(service, 0, new Intent()
      .setClassName(service, "com.github.shadowsocks.Shadowsocks").setFlags(Intent.FLAG_ACTIVITY_REORDER_TO_FRONT), 0))
